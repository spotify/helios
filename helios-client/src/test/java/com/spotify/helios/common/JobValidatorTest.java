...
public class JobValidatorTest {
  private final HealthCheck HEALTH_CHECK =
    HealthCheck.newBuilder().setType(HealthCheck.HTTP).setPath( "/" ).setPort("1").build();
  ...
  public void testInvalidHealthCheckFail() {
    final Job j = Job.newBuilder().setName( "foo" ).setVersion( "1" ).
      setImage( "foobar" ).setHealthCheck( HEALTH_CHECK ).build();
    assertEquals(newHashSet("Health check port '1' is unknown to job. Known ports are ''"),
                 validator.validate(j));
    assertEquals(newHashSet("Health check port '1' is unknown to job. Known ports are 'a'"),
                 validator.validate(j.toBuilder().addPort("a", PortMapping.of(1, 1)).build()));
  }
