...
public class RollingUpdateCommandTest {
  ...
  public void testRollingUpdate() throws Exception {
    ...
    verify(client).rollingUpdate(
        GROUP_NAME, JOB_ID,
        new RolloutOptions(TIMEOUT, PARALLELISM, false, false, FAILURE_THRESHOLD));
    assertEquals(0, ret);

    final String expected = (
        "Rolling update started: my_group -> foo:2:1212121 (parallelism=1, timeout=300, "
        + "overlap=false, failure threshold=1)\n" +
        "\n" +
        "host1 -> RUNNING (1/3)\n" +
        "host2 -> RUNNING (2/3)\n" +
        "host3 -> RUNNING (3/3)\n" +
  ...
  public void testRollingUpdateMigrate() throws Exception {
    ...
    verify(client).rollingUpdate(
        GROUP_NAME, JOB_ID,
        new RolloutOptions(TIMEOUT, PARALLELISM, true, false, FAILURE_THRESHOLD));
    assertEquals(0, ret);
  ...
  public void testRollingUpdateOverlapJson() throws Exception {
    ...
    verify(client).rollingUpdate(
        GROUP_NAME, JOB_ID,
        new RolloutOptions(TIMEOUT, PARALLELISM, false, true, FAILURE_THRESHOLD));
    assertEquals(0, ret);
