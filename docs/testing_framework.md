...
A sample `reference.conf` file (or `application.conf`, etc.  See the
Typesafe docs on ways to configure from whence it reads its config
files) you could put in your projects
...
You can also consult the [`application.conf`](../helios-testing/src/test/resources/application.conf) file for an example.
